...
const monthCounts = async (data) => {
  ...
  try {
    const [ordersCount, resellersCount] = await Promise.all([
      Order.aggregate([
        ...
        { 
          ...
          // Group by customerName to get unique customers
          { 
            $group: { 
              _id: "$customers.customerName" 
            } 
          },
          { 
    ...
    const totalOrders = ordersCount.length > 0 ? ordersCount[0].totalCustomers : 0;
    const totalResellers =
      resellersCount.length > 0 ? resellersCount[0].totalResellers : 0;
