...
const ListBadges = ({
  ...
  const availabilityData = useCheckAvailabilityForCustomization(
    selectedOrder._id,
    customerId,
    selectedSize
  );
  ...
  const handleSubmit = () => {
    ...
    const totalPrice = selectedBadges.reduce(
      (sum, badge) => sum + badge.price,
      0
    );
    ...
    dispatch(
      addBadges({
        badges: [
          {
            size: selectedSize,
            badges: selectedBadges.map((item) => item._id),
          },
        ],
  ...
  const isBadgeAvailableForSize = (badge) => {
    const selectedCount = selectedBadges?.filter(
      (b) => b._id === badge._id && b.size === selectedSize
    ).length;
    const availableStock =
      availabilityData?.customizationsForSize?.find(
        (customization) => customization.badgeId === badge._id
      )?.stock || 0;
        ...
        {badges.map((badge) => {
          ...
          return (
            <Menu.Item
              ...
              disabled={!badge.stock}
            >
              <div
                className={`cursor-pointer flex ${
                  isBadgeDisabled ? "bg-gray-300" : "bg-blue-100"
                } items-center w-[300px] border rounded-lg p-2`}
              >
                <img
                  alt={badge.name}
                ...
                {!badge.stock && (
                  <div className="text-red-500 ms-auto">Out of stock</div>
                )}
                {isSelected && (
                  <IoCloseCircle className="ms-auto text-red-600" />
                )}
              </div>
            </Menu.Item>
          );
